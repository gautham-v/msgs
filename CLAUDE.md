...
- `src/theme.rs` — named color slots (mockup palette, plus `Theme::light`), `Base` (dark / light / system, from `[theme] base`, `--theme`, or `Ctrl+T`), `system_is_dark` (`defaults read -g AppleInterfaceStyle`), overridable per slot from config
