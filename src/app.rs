...
pub struct App {
    ...
    /// Files the `@` picker has queued, drawn as chips above the draft and
    /// sent ahead of it when `Enter` sends.
...
impl App {
    ...
    fn send_composed(&mut self) {
        let nothing_typed = self.composer.text().trim().is_empty();
        if nothing_typed && self.attached.is_empty() {
            return;
        }
        ...
        // Files go first, each its own message, so the words that were typed
        // about them land underneath rather than above.
    ...
    fn composer_lines(&self) -> u16 {
        if let Some(prompt) = self.attach_prompt.as_ref() {
            return prompt.line_count();
        }
        // Each queued file is a chip on a row of its own above the draft.
        let chips = u16::try_from(self.attached.len()).unwrap_or(u16::MAX);
        self.composer.line_count().saturating_add(chips)
    }
}
