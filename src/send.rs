...
//!
//! Nothing here logs. Errors coming back from `osascript` are run through
//! [`sanitize`], which drops the quoted spans — the ones that would otherwise
//! carry a phone number or a body onto the status line.
...
pub fn sanitize(stderr: &str) -> String {
    let line = stderr
        ...
        .unwrap_or("Messages refused the message");
    let line = line
    ...
    let out = out.trim();
    if out.is_empty() {
        return "Messages refused the message".to_string();
    }
